...
\subsection{主要和次要热传输路径}\label{sec:path}
大多现在VLSI系统的封装有若干不同材料的叠层组成，如图 \ref{fig/layers}。
\pic{ 一维热传导}{width=0.7\columnwidth}{fig/layers}
HotSpot热模型就以此为例来说明。
典型的层有：散热片，散热层，热界面材料（导热膏），硅衬底，芯片互连层，C4垫，陶瓷封装衬底，焊料球等。
堆叠芯片封装（SCP）和 3D IC设计也是堆叠分层结构，可以作为通用堆叠结构的扩展很容易建模，如图 \ref{fig/path}。
\pic{ 一维热传导}{width=0.5\columnwidth}{fig/path}
...
有这些量之后，就可以通过~\cite{song1995constriction} 中的公式来计算扩散热组。
